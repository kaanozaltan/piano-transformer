...
def evaluate_mgeval_combined(
    ...
    absolute_summary = []
    for i in range(len(absolute_summary_train)):
        mean_train = absolute_summary_train[i]["Mean"]
        if np.isscalar(mean_train) or (
            isinstance(mean_train, np.ndarray) and mean_train.ndim == 0
        ):
            mean_generated = absolute_summary_generated[i]["Mean"]
            std_train = absolute_summary_train[i]["Std"]
            std_generated = absolute_summary_generated[i]["Std"]
            absolute_summary.append(
                {
                    "Feature": absolute_summary_train[i]["Feature"],
                    "Rel_Diff_Mean": abs(mean_generated - mean_train) / mean_train,
                    "Rel_Diff_Std": abs(std_generated - std_train) / std_train,
                }
            )
    ...
    if output_path is not None:
        for i in range(len(features)):
            ...
            figure_path = os.path.join(
                output_path, "graphics", f"{features[i]}_distance_plot.png"
            )
            plt.savefig(figure_path)
            plt.clf()
...
def fmd(ref_dir, gen_dir):
    ...
    return fmd_metric.score(reference_path=ref_dir, test_path=gen_dir)


# TOKENIZER_PATH = "/hpcwork/lect0148/experiments/mistral-162M_remi_maestro_v1/tokenizer.json"  # adapt path for local use

# def extract_features(
#     directory, tokenizer_path=TOKENIZER_PATH, features=("pitch", "duration", "velocity")
# ):
